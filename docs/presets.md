...
- [`examples/agent-preset.json`](../examples/agent-preset.json) stores a Sovereign Agent CLI run draft for precompile `0x080C`. Replace the zero executor; the studio replaces the example callback with the connected wallet's predicted factory child.
