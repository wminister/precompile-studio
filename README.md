...
After the wallet-owned child exists, the studio discovers a capability-0 executor and public key from `TEEServiceRegistry`, encrypts the credential-free Ritual provider configuration in the browser, and sets two-phase delivery to that child. **Run once** sends the encoded request directly to precompile `0x080C`, pays from the connected wallet's existing RitualWallet escrow, and does not use the Scheduler reserve. **Recurring** submits `configureFundAndStart` from the harness owner and funds a rolling Scheduler window.

The launch creates a rolling window of up to five calls every 2,000 blocks and locks harness funding for 100,000 blocks. Ritual Scheduler requires a fixed `0.01 RITUAL` reserve plus the initial window's complete execution budget. The studio treats the entered amount as execution funding, derives the encoded window size from it, and adds the reserve automatically to the wallet transaction. At the default `5,000,000` gas and `2 gwei` fee cap, `0.01 RITUAL` funds one call and produces a `0.02 RITUAL` launch transaction; `0.03` funds three calls for a `0.04` total; and `0.05` funds the five-call maximum for a `0.06` total. The fee cap is editable and must remain at or above the live RPC gas-price suggestion. This harness escrow is separate from the connected wallet's existing RitualWallet escrow. The studio reads the harness owner, series state, and sender lock, then reconciles `JobAdded`, `Phase1Settled`, `ResultDelivered`, `JobRemoved`, and the harness callback event into user-facing lifecycle states. Scheduler funding and transaction gas are paid by the wallet that confirms the launch.

Live Sovereign Agent wallet submission is currently circuit-broken. A verified provider-failed request charged `0.50784 RITUAL` as an executor/model fee even though no reliable pre-signing quote was available. Agent composition, registry discovery, ABI export, and historical trace inspection remain available; one-shot and recurring wallet actions stay disabled until the application can verify a maximum executor/model charge before signing.
