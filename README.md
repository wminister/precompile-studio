...
HTTP, JQ, and Scheduled JQ are publicly usable. Scheduled JQ discovers or creates one deterministic contract owned by the connected wallet. Sovereign Agent composition and lifecycle inspection remain available, but paid Agent launch is paused after accepted Scheduler calls were not dispatched. LLM submission is implemented but currently depends on a degraded Ritual executor path.
...
The Studio reads harness ownership, series state, and sender lock, then reconciles `JobAdded`, `Phase1Settled`, `ResultDelivered`, `JobRemoved`, and the harness callback event into user-facing lifecycle states. Paid launch is currently disabled. The last browser attempts used low Scheduler fee caps and were accepted without being dispatched. The exact historical profile that did execute used an `800,000` callback gas limit, `20 gwei` max fee, `1 gwei` priority fee, one call after 180 blocks, and a `0.1 RITUAL` harness deposit. That deposit funded the harness; it was not a quoted execution fee.

Direct Sovereign Agent submission is circuit-broken after a provider-failed request charged `0.50784 RITUAL` without a reliable pre-signing quote. Historical native-GLM harnesses using executor `0x9dc1...8b4C` completed, but the newer low-fee Scheduler profiles did not dispatch. The Studio therefore keeps Agent encoding, discovery, and evidence visible while refusing all paid browser launches until the historical profile is reproduced end to end from the browser.
