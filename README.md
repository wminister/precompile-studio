...
The launch creates one call, checks the editable Scheduler fee cap against the live RPC suggestion, and locks harness funding for 100,000 blocks. The transaction sends exactly `0.02 RITUAL`: `0.01 RITUAL` execution funding plus Ritual Scheduler's `0.01 RITUAL` reserve. That harness escrow is separate from the connected wallet's existing RitualWallet escrow, so the wallet confirmation shows the complete maximum value exposed to the Agent run. The studio reads the harness owner, series state, and sender lock, then reconciles `JobAdded`, `Phase1Settled`, `ResultDelivered`, `JobRemoved`, and the harness callback event into user-facing lifecycle states.

Direct Sovereign Agent submission is circuit-broken after a provider-failed request charged `0.50784 RITUAL` without a reliable pre-signing quote. Historical native-GLM harnesses using executor `0x9dc1...8b4C` completed at roughly `0.01 RITUAL` per execution. The Studio therefore enables only the isolated one-call recurring profile above and refuses live launch if the model, runtime, executor, registry validity, or exact `0.02 RITUAL` transaction ceiling changes.
