...
HTTP, JQ, Scheduled JQ, and the wallet-owned recurring Sovereign Agent harness are publicly usable. Scheduled JQ discovers or creates one deterministic contract owned by the connected wallet. Agent launch uses a locally simulated one-call configuration with Ritual's documented Scheduler fee floor. LLM submission is implemented but currently depends on a degraded Ritual executor path.
