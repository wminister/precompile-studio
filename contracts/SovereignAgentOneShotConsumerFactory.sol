...
import {UserSovereignAgentOneShotConsumer} from "./SovereignAgentOneShotConsumer.sol";

/// @notice Deploys one deterministic one-shot Agent consumer per wallet.
contract SovereignAgentOneShotConsumerFactory {
    ...
    function createConsumer() external returns (address consumer) {
        ...
        consumer = address(new UserSovereignAgentOneShotConsumer{salt: _salt(msg.sender)}(scheduler, msg.sender));
    ...
    function predictConsumer(address owner) external view returns (address) {
        bytes memory creationCode =
            abi.encodePacked(type(UserSovereignAgentOneShotConsumer).creationCode, abi.encode(scheduler, owner));
    ...
    function _salt(address owner) private pure returns (bytes32) {
        return keccak256(abi.encodePacked("PRECOMPILE_STUDIO_AGENT_ONE_SHOT", owner));
    }
}
